...
SELECT nombre_jugador from jugadores where id_equipo = 2;
...
SELECT count(*) as numero_jugadores from jugadores where id_equipo = 2;
...
SELECT id_equipo, AVG(edad) AS edad_promedio
FROM jugadores
GROUP BY id_equipo
ORDER BY id_equipo ASC, edad_promedio DESC;



-- 13. Mostrar el promedio de goles y asistencias de cada equipo, con alias "promedio_goles" 
-- y "promedio_asistencias", ordenados por promedio de goles descendente y promedio de asistencias ascendente.
SELECT
    id_equipo,
...
ORDER BY 
    promedio_goles DESC,
    promedio_asistencias ASC;

-- 14. Obtener los equipos con el total de goles anotados en sus partidos, 
-- usando alias "total_goles", ordenados por nombre del equipo ascendente y total de goles descendente.
SELECT
    id_equipo_local AS nombre_equipo,
...
-- 15. Contar la cantidad de partidos jugados por cada equipo local, con alias "partidos_local", 
-- ordenados por estadio y cantidad de partidos descendente.


-- 16. Calcular la cantidad de partidos en los que el equipo local ganó, 
-- usando alias "victorias_local", ordenados por cantidad de victorias y 
-- goles locales promedio en orden descendente.